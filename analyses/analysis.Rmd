...
par(mfrow=c(1,2), mai = c(.9, .5, .5, 1.75), mar=c(0,1,2,6),cex.main=.8)
...
title("16S rRNA Assignments",line = -.5, adj=0, cex=.002)
dev.off()
...
par(mfrow=c(1,2), mai = c(.9, .5, .5, 1.75), mar=c(0,1,2,5),cex.main=.8)

dendextend::set_labels(protein_peak_dend$dendrogram,
                       search_result$genus[match(labels(protein_peak_dend$dendrogram),
                                                 search_result$query)]) %>%
    dendextend::set("labels_cex", 0.75) %>%
    plot(ylim= c(600,851), horiz=TRUE)
